...
class SharedRequestManager {
    ...
    async executeRequest(request) {
        ...
        while (request.retries <= request.maxRetries) {
            ...
            // Native fetch doesn't support 'agent' option
            ...
            try {
                ...
                if (response.status === 304) {
                    this.log(`✓ Resource not modified (304) for ${request.moduleId}: ${request.url}`);
                    return { success: true, status: 304, headers: Object.fromEntries(response.headers.entries()) };
                }
